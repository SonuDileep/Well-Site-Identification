...
cfg = '/mnt/Data/yolov4_ubuntu/custom_v4/yolov4(2).cfg'
data = '/mnt/Data/yolov4_ubuntu/custom_v4/detector.data'
weights = '/mnt/Data/yolov4_ubuntu/custom_v4/yolov4_6000.weights'
...
def find_well_pad_tank(points,p2): # find the coordinates between the range of points
 ...
 #   print(x_min,x_max,y_min,y_max)
    add_1 = 370 #size of the small block of image
    add_2 = 360 # size of the large block of image
    x1 = x_min
    x2 = x1 + add_1
    ...
    while x1<=x_max:
        ...
        flag = 0
        while y1>=y_max:
            if poly.geometry().contains(QgsPointXY(x1, y1)) \
                    == False:  # Checks if the rectangle is inside our search space
                y1 = y1 - add_2
                y2 = y2 - add_2
                continue 
            row = row + 1
...
#                print(c)
 ...
 #               draw_layer(x1,y1,x2,y2,'well_pad')
        ...
        x1 = x1 + add_2
        x2 = x2 + add_2
    well_pad_1 = inter_contain(layers_list)
    pad1 = copy.deepcopy(well_pad_1)

    with open('/home/sonu/Desktop/test/wp_300.txt', "w") as f:
        json.dump(pad1, f)
    del layers_list[:]
    print("Done-1")
################################ A larger tile #############################
...
#    poly = draw_range(points)
 #   print(x_min,x_max,y_min,y_max)
    add_1 = 750 #size of the small block of image
    add_2 = 700 # size of the large block of image
    x1 = x_min
    x2 = x1 + add_1
    img = 0 # to get image count
    row = 0
 ...
 #               draw_layer(x1,y1,x2,y2,'well_pad')
        ...
        x1 = x1 + add_2
        x2 = x2 + add_2
    well_pad_2 = inter_contain(layers_list)
    well_pad_2.extend(pad1)
    final_well_pad = inter_contain(well_pad_2)     
    with open('/home/sonu/Desktop/test/wp_finall.txt', "w") as f:
        json.dump(final_well_pad, f)    
#    addlayer_from_list(layers_list,"well_pad")
    print("Done-2")
##### Search for well pad around well heads
#    final_well_pad = []
    with open('/home/sonu/Desktop/test/around_well_head.txt') as f:
        temppp = json.load(f)
...
#            if sizes[l] == 150:
#                threshold = 0.20
#            c = detect(img_loc,threshold)
#            if c!=False:
#                m1=[]
#                m1 = find_layer(c,img_loc,[x11,y11,x22,y22],img,w,h)
#                for j in range(len(m1)):
#                    t_list.append(m1[j])
#        t_list = inter_contain(t_list)
#        layers_list_1.extend(t_list)               
#    layers_list_1 = inter_contain(layers_list_1)
#    with open('/home/sonu/Desktop/test/around_well_head.txt', "w") as f:
#        json.dump(layers_list_1, f)
#        
    temppp.extend(final_well_pad)
    final_well_pad2 = inter_contain(temppp)   
    with open('/home/sonu/Desktop/test/complete_pad_list.txt', "w") as f:
        json.dump(final_well_pad2, f)  
#
    addlayer_from_list(final_well_pad2,"well_pad")
